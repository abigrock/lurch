            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for (fi, &(orig_idx, m)) in filtered.iter().enumerate() {
                        ...
                        let base_name = m.filename.strip_suffix(".disabled").unwrap_or(&m.filename);
                        ...
                        let row_resp = ui.allocate_ui_with_layout(
                            ...
                            |ui| {
                                ...
                                let project_url = if (m.source == "modrinth"
                                    || m.source == "curseforge")
                                ...
                                if has_update {
                                    ...
                                    badge_inner.response.on_hover_text(format!(
                                        "Update → {}",
                                        self.mod_updates
                                            .get(base_name)
                                ...
                                ui.with_layout(
                                    ...
                                    |ui| {
                                        ...
                                        ui.add(
                                            egui::Label::new(theme.subtext(&m.filename)).truncate(),
                                        );
                                    },
                                );
                            },
                ...
                "curseforge" => {
                    ...
                    std::thread::spawn(move || {
                        let result = (|| -> anyhow::Result<ModInstallResult> {
                            ...
                            let new_filename = curseforge::download_cf_file(file, &mods_path)?;
                            if new_filename != old_filename {
                                let _ = std::fs::remove_file(mods_path.join(&old_filename));
                            }
                            Ok(ModInstallResult {
